...
flag = 0                                    #flag utilizada para indicar presença ou não de rosto na imagem para ligar e desligar o led

GPIO.setmode (GPIO.BCM)                     #Estabelece a referência de numeração da pinagem baseado no Chip (BCM)
GPIO.setwarnings(False)

GPIO.setup(26,GPIO.OUT)                     #Estabelece o pino GPIO26 como saída (LED)
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        GPIO.output(26,GPIO.HIGH)                   #Acende o LED no evento de detecção de rosto
        flag = 1                                    #muda o flag para 1
        i = 0                                       #zera o índice auxiliar para contar iterações do laço while 
        print ("Rosto Detectado")                   #mostra no terminal que um rosto foi detectado
    ...
    if flag == 1:
        i=i+1
        if i >= 10:
            GPIO.output(26,GPIO.LOW)            #apaga o LED após 10 iterações do laço while consecutivas com o flag igual a 1
            flag = 0                            #zera a flag
